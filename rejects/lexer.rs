...
pub mod types{
  ...
  #[derive(Debug,PartialEq)]
  pub struct TokenIdentifier<'a>(&'a str);
  impl TokenIdentifier<'_> {
    pub fn new<'a>(val: &'a str) -> TokenIdentifier<'_>{
...
pub mod tokenizer {
  use crate::{lexer::{
    eval::{comp_key, comp_operator, comp_pun, is_token_oper, is_token_sep}, 
    types::{LexerCartegories, TokenIdentifier, TokenKeyword, TokenOperator, TokenSeparator}}};
  ...
  pub fn tokenize(chars: Vec<char>) {
    ...
    for (i, char) in chars.iter().enumerate() {
      if char.is_alphabetic() || !keyword_value.is_empty() && char.is_numeric(){
        ...
        let value = LexerCartegories::Unknown(*char);
        tokens.push(value);
        //println!("Found smth else: {:?}", char);
      }
    }
    flush_keyword(&mut keyword_value, &mut tokens);
    tokens.push(LexerCartegories::EndOfFile);
    println!("\n{:?}",tokens.blue().bold());
  }
  fn flush_keyword(keyword: &mut Vec<char>, tokens: &mut Vec<LexerCartegories<'_>>){
    if keyword.is_empty() {
      return;
    } 
    ...
    let value = match comp_key(val.as_str()){
      ...
      //None => LexerCartegories::Identifier(String::from(val)),
